...
const safe = (a) => {
    const { passwordHash, passwordSalt, ...x } = a;
    return x;
};
const hash = (p, s = crypto.randomBytes(16).toString("hex")) => ({
    passwordHash: crypto.scryptSync(p, s, 64).toString("hex"),
    passwordSalt: s,
});
const app = express();
...
app.use(
    session({
        secret:
            ...
            "plantist-development-secret-change-me",
        resave: false,
...
const auth = (req, res, next) =>
    me(req) ? next() : res.status(401).json({ error: "Please log in first." });
app.post("/api/auth/signup", upload.single("profile"), (req, res) => {
    ...
    const d = read();
    if (d.accounts.some((a) => a.username === username))
        return res
            .status(409)
    ...
    const h = hash(password),
        a = {
            ...
            "Plant Profiles": [],
            "Plant Protocols Scale": { level: 1, notes: "" },
            "Plant Panel Items": [],
        };
    d.accounts.push(a);
    write(d);
    req.session.accountId = a.id;
    res.status(201).json({
...
app.post("/api/auth/login", (req, res) => {
    const username = String(req.body.username || "")
            ...
            .toLowerCase(),
        password = String(req.body.password || ""),
        a = read().accounts.find((x) => x.username === username);
    if (!a)
        return res
            .status(401)
            .json({ error: "That username or password is not correct." });
    const h = hash(password, a.passwordSalt);
    if (
        !crypto.timingSafeEqual(
            Buffer.from(h.passwordHash, "hex"),
        ...
        return res
            ...
            .json({ error: "That username or password is not correct." });
    req.session.accountId = a.id;
    res.json({ message: "welcome back.", account: safe(a) });
});
...
app.get("/api/auth/me", (req, res) => {
    const a = me(req);
    a
        ? res.json({ account: safe(a) })
        : res.status(401).json({ error: "Not logged in." });
});
app.post("/api/plants", auth, upload.array("images", 12), (req, res) => {
    const d = read(),
        a = d.accounts.find((x) => x.id === req.session.accountId),
        age = Math.max(
            0,
            parseInt(req.body.manuallyAddedAgeDays || "0", 10) || 0,
        ),
        ...
        p = {
            id: id("plant"),
            name: String(req.body.name || "").trim(),
            speciesType: String(req.body.speciesType || "").trim(),
            description: String(req.body.description || "").trim(),
            manuallyAddedAgeDays: age,
            ageStartedAt: new Date().toISOString(),
            additionalSpecs: {
                color: String(req.body.color || "").trim(),
                size: String(req.body.size || "").trim(),
                other: String(req.body.otherSpecs || "").trim(),
            },
            images: imgs,
            timeline: imgs.map((i) => ({
                imageId: i.id,
    ...
    a["Plant Profiles"].push(p);
    write(d);
    res.status(201).json({ plant: p });
});
...
app.post("/api/protocols", auth, (req, res) => {
    const d = read(),
        a = d.accounts.find((x) => x.id === req.session.accountId);
    a["Plant Protocols Scale"] = {
...
app.listen(Number(process.env.PORT) || 3001, () =>
    console.log(
        "plantist server running on port " + (process.env.PORT || 3001),
    ),
);
